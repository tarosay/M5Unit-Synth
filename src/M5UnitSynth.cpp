...
void M5UnitSynth::begin(HardwareSerial *serial, int baud, uint8_t RX, uint8_t TX) {
  _hwSerial = serial;
  _swSerial = nullptr;
  _hwSerial->begin(baud, SERIAL_8N1, RX, TX);
}

void M5UnitSynth::begin(SoftwareSerial *serial, int baud, uint8_t RX, uint8_t TX) {
  _swSerial = serial;
  _hwSerial = nullptr;
...
void M5UnitSynth::sendCMD(uint8_t *buffer, size_t size) {
  if (_hwSerial) {
    _hwSerial->write(buffer, size);
  } else if (_swSerial) {
    _swSerial->write(buffer, size);
  }
}
